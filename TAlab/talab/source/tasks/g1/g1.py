...
class RobotSceneCfg(InteractiveSceneCfg):
    ...
    terrain = TerrainImporterCfg(
        ...
        terrain_generator=PLANE_TERRAINS_CFG,  # PLANE_TERRAINS_CFG, ROUGH_TERRAINS_CFG
        max_init_terrain_level=PLANE_TERRAINS_CFG.num_rows - 1,
        collision_group=-1,
...
class EventCfg:
    ...
    #     "friction_distribution_params": (0.0, 0.05),    # 关节摩擦轻微噪声
    #     "armature_distribution_params": (0.8, 1.2),     # 质量矩微扰
    #     "operation": "scale",
    #     },
    # )

    # gravity_jitter = EventTerm(
    # func=mdp.randomize_physics_scene_gravity,
    # mode="startup",
    # params={
    #     "gravity_distribution_params": ( [0.0, 0.0, -0.5], [0.0, 0.0, 0.5] ),  # ±0.5 m/s^2
    #     "operation": "add",
    #     },
    # )
    ...
    #     params={
    #         "asset_cfg": SceneEntityCfg("robot", joint_ids=slice(None)),
    #         "stiffness_distribution_params": (0.9, 1.1),  # scale/abs看实现，这里配合 operation="scale"
    #         "damping_distribution_params": (0.9, 1.1),
    #         "operation": "scale",
    #     },
    # )

    # com_jitter = EventTerm(
    ...
    #         "contact_offset_distribution_params": (0.0, 0.003),
    #         "distribution": "uniform",
    #     },
    # )
    
    # reset_from_terrain = EventTerm(
    #     func=mdp.reset_root_state_from_terrain,
    #     mode="reset",
    #     params={
    #         "pose_range": {"roll": (-0.02, 0.02), "pitch": (-0.05, 0.0), "yaw": (-3.14, 3.14)},
    ...
    reset_base = EventTerm(
        ...
        mode="reset",
        params={
            "pose_range": {"x": (-0.5, 0.5), "y": (-0.5, 0.5), "yaw": (-3.14, 3.14), "pitch": (-0.05, 0.0)},
            "velocity_range": {
                "x": (0.0, 0.0),
                "y": (0.0, 0.0),
                "z": (0.0, 0.0),
    ...
    #     params={"velocity_range": {"x": (-0.8, 0.8), "y": (-0.4, 0.4)}},
    # )
...
class CommandsCfg:
    ...
    base_velocity = mdp.UniformLevelVelocityCommandCfg(
        ...
        ranges=mdp.UniformLevelVelocityCommandCfg.Ranges(
            lin_vel_x=(-0.5, 2.0), lin_vel_y=(-0.5, 0.5), ang_vel_z=(-1.57, 1.57), heading=(-math.pi, math.pi)
        ),
...
class ObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        ...
        base_ang_vel = ObsTerm(func=mdp.base_ang_vel, scale=0.25, noise=Unoise(n_min=-0.15, n_max=0.15))
        projected_gravity = ObsTerm(func=mdp.projected_gravity, noise=Unoise(n_min=-0.03, n_max=0.03))
        velocity_commands = ObsTerm(func=mdp.generated_commands, params={"command_name": "base_velocity"}, noise=Unoise(n_min=-0.02, n_max=0.02))
        joint_pos_rel = ObsTerm(func=mdp.joint_pos_rel, noise=Unoise(n_min=-0.01, n_max=0.01))
        joint_vel_rel = ObsTerm(func=mdp.joint_vel_rel, scale=0.1, noise=Unoise(n_min=-0.5, n_max=0.5))
...
class RewardsCfg:
    ...
    joint_deviation_ankels = RewTerm(
        func=mdp.joint_deviation_l1,
        weight=-2.0,
